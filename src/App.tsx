...
const BackgroundVideo = React.forwardRef<HTMLVideoElement, BackgroundVideoProps>(
  ({ src, isActive, isIntro, isMuted = true, shouldLoop = true, onTimeUpdate, onEnded, onLoadedData }, externalRef) => {
...
// Extracted to src/GateScene.tsx
      ...
      <div className="relative w-full h-full rounded-[2rem] sm:rounded-[2.5rem] bg-black z-10 shadow-2xl border border-white/10">
        ...
        <AnimatePresence>
          {stage !== 'profile' && (
            ...
            >
              <Suspense fallback={
                <div className="flex items-center justify-center text-white/50 text-2xl font-bold tracking-widest">
                  THE GT
                </div>
              }>
                <GateSceneCanvas isTransitioning={stage === 'transition'} />
              </Suspense>
